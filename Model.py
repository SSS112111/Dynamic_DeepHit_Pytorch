...
class GRU_With_Dropout(nn.Module):
    # Initialization function for the GRU_With_Dropout module
    def __init__(self, input_size, hidden_size, dropout, bias=True):
        ...
        self.fc_net = FC_net(input_size=hidden_size*2+(input_size-2),output_size=1,hidden_size=hidden_size,dropout=dropout,activ_out=None,nlayers=2,activation='Tanh')
...
class DynamicDeepHitTorch(nn.Module):
    ...
    def __init__(self, input_dim, output_dim,network_settings, risks,optimizer='Adam'):
        ...
        self.rnn = GRU_With_Dropout(input_size=input_dim*2,
        ...
        for r in range(self.risks):
            self.cause_specific.append(FC_net(input_size=self.h_dim2, output_size=self.h_dim2,activ_out= self.FC_active_fn,
    ...
    def forward(self, x,x_mi): # x represent the data and x_mi the mask data
        ...
        x_last = torch.sum(torch.unsqueeze(rnn_mask2, dim=2).repeat(1, 1, feature_length) * x, dim=1)  # Sum over time, since all other time stamps are 0
        x_last = x_last[:, 1:]  # Remove delta of last measurement
        x_hist = x * (~torch.unsqueeze(rnn_mask2, dim=2).repeat(1, 1,feature_length))  # Since all other time stamps are 0 and measurements are 0-padded
        x_hist = x_hist[:, :max_length - 1, :]  # Only take history up to the last time step

        x_mi_last = torch.sum(torch.unsqueeze(rnn_mask2, dim=2).repeat(1, 1, feature_length) * x_mi,dim=1)  # Sum over time, since all other time stamps are 0
        x_mi_last = x_mi_last[:, 1:]  # Remove delta of last measurement
        x_mi_hist = x_mi * (~torch.unsqueeze(rnn_mask2, dim=2).repeat(1, 1,feature_length))  # Since all other time stamps are 0 and measurements are 0-padded
